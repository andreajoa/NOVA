        ...
        <div className="mt-5 rounded-[2rem] border border-white/10 bg-[#070707] p-6">
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[.025] p-4 text-xs text-white/40">Custo: {seconds * 24} creditos</div>
              <button onClick={handleExtend} disabled={loading} className="min-h-14 rounded-2xl bg-[#D7FF00] px-6 py-4 text-sm font-black uppercase text-black disabled:opacity-45">
                {loading ? (status || "Gerando...") : ("Estender " + seconds + "s")}
              </button>
            </div>
          </div>
        ...
        {result && (
          <div className="mt-5 rounded-[2rem] border border-white/10 bg-[#070707] p-6">
            ...
            <div className="mt-3 flex gap-3">
              ...
              <a href={"/api/download?url=" + encodeURIComponent(result.videoUrl) + "&filename=nova-extend"} className="flex-1 rounded-xl bg-[#D7FF00] px-4 py-3 text-center text-xs font-black uppercase text-black no-underline">Baixar</a>
            </div>
          </div>
        )}
      </div>
