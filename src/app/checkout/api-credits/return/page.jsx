...
function ReturnContent() {
  ...
  const sessionId = searchParams?.get("session_id") || "pending";
  ...
  return (
    <main className="min-h-screen bg-[#050505] px-5 py-16 text-white">
      <div className="mx-auto max-w-2xl rounded-[32px] border border-[#D7FF00]/35 bg-[#D7FF00]/10 p-8 text-center">
        ...
        <p className="mt-5 text-xs text-white/25">Session: {sessionId}</p>
      </div>
    </main>
  );
}

export default function ApiCreditsReturnPage() {
  return (
    <Suspense>
      <ReturnContent />
    </Suspense>
