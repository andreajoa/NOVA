...
const navGroups = [
  { label: null, items: [
      ...
      { href: "/dashboard/free",               label: "Gerar Grátis", icon: "⚡", accent: true },
...
const mobileItems = [
  ...
  { href: "/dashboard/free",     label: "Grátis",   icon: "⚡", accent: true },
  { href: "/dashboard/generate", label: "Vídeo",    icon: "▶" },
  { href: "/dashboard/models",   label: "Imagem",   icon: "⬛" },
];
    ...
    <div className="mb-1">
      {group.label && (
        ...
        >
          ...
          {group.collapsible && (
            <span className="text-[10px] text-white/20 transition-transform duration-200" style={{ display: "inline-block", transform: open ? "rotate(90deg)" : "rotate(0deg)" }}>›</span>
          )}
        </button>
      )}
      {open && (
        <div className="space-y-0.5">
          {group.items.map((item) => {
            ...
            const accented = item.accent && !active;
            return (
              <Link
                key={item.href}
                href={item.href}
  ...
  return (
    <div className="border-b border-white/[0.06] last:border-0">
      <button onClick={() => setOpen((v) => !v)} className="flex w-full items-center justify-between px-1 py-3">
        ...
        <span className="text-sm text-white/25 transition-transform duration-200" style={{ display: "inline-block", transform: open ? "rotate(90deg)" : "rotate(0deg)" }}>›</span>
      </button>
      {open && (
    ...
    <>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm lg:hidden" onClick={() => setMobileOpen(false)}>
          <div className="absolute bottom-[76px] left-3 right-3 max-h-[70vh] overflow-y-auto rounded-[1.5rem] border border-white/10 bg-[#050505] shadow-[0_0_80px_rgba(215,255,0,.12)]" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-[1.5rem] border-b border-white/10 bg-[#050505] px-5 py-4">
              ...
              <button onClick={() => setMobileOpen(false)} className="rounded-full border border-white/10 px-3 py-1 text-xs font-black text-white/50">Close</button>
            </div>
            <div className="px-4 pb-2">
              {mobileDrawerGroups.map((group, i) => <DrawerAccordionGroup key={i} group={group} pathname={pathname} onClose={() => setMobileOpen(false)} />)}
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 grid grid-cols-5 border-t border-white/10 bg-[#050505]/95 backdrop-blur-md lg:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        ...
        <button type="button" onClick={() => setMobileOpen((v) => !v)} className={["flex flex-col items-center gap-1 px-1 py-3 transition", mobileOpen ? "text-[#D7FF00]" : "text-white/35 hover:text-white"].join(" ")}>
