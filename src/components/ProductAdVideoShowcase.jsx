...
function SmartLoopVideo({ src, title }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-[1.6rem] border border-white/10 bg-black shadow-[0_30px_90px_rgba(0,0,0,.45)] transition duration-500 hover:-translate-y-1 hover:border-[#D7FF00]/50">
      <video
        ref={videoRef}
        src={src}
        className="aspect-[9/16] w-full object-cover opacity-90 transition duration-700 group-hover:scale-[1.04] group-hover:opacity-100"
        muted
        loop
        playsInline
        preload="metadata"
...
export default function ProductAdVideoShowcase() {
  return (
    <section className="relative overflow-hidden bg-[#030303] px-4 py-14 text-white md:px-8 md:py-20">
        ...
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-white/50 md:text-base">
              Real examples of product-style creatives, UGC vibes and ad visuals
              powered by NOVA.
            </p>
          </div>
