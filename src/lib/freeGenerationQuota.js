...
function monthlyPeriodUTC(date = new Date()) {
  return `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, "0")}`;
}

function limitFor(kind) {
  const envName = kind === "video"
    ? "NOVA_FREE_VIDEO_MONTHLY_LIMIT"
    : "NOVA_FREE_IMAGE_MONTHLY_LIMIT";
...
export async function checkAndDebitFreeGeneration(userId, kind) {
  ...
  if (limit <= 0) {
    return { ok: false, kind, period, used: 0, limit, remaining: 0 };
  }
